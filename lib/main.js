(function() {
  ...
  this.permute = function(me) {
    ...
    /* TAINT consider to update or delete factors instead */
    var factors, i, infix, infix_idx, j, len, permutation_count, permutations, prefix, r_idx, r_weights, ref, ref1, suffix, target, weight_idx, weights;
    if (me['weights'] == null) {
    ...
    for (weight_idx = i = 0, len = ref.length; i < len; weight_idx = ++i) {
      ...
      for (infix_idx = j = 0, ref1 = permutation_count; 0 <= ref1 ? j < ref1 : j > ref1; infix_idx = 0 <= ref1 ? ++j : --j) {
